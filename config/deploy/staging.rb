...
server 'accruby02.cg.local', user: 'deploy', runner: 'deploy', group: 'deploy', roles: %w{app web db}

# Asset Server
# A server with the [worker] role copies the precompiled data from the precompile server
set :precompile_server, 'accruby02.cg.local'
...
set :passenger_rvm_ruby_version, 'ruby-2.4.0@global'
